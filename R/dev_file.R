bivqfun <- function(data, var1, var2, tau = 0.5, nalpha = 100, biiter=250, plot = TRUE) {
  ...
  bivqcurve <- data.frame(
    var1 = seq(min(data[[var1]]), max(data[[var1]]), length = biiter),
    y1 = seq(min(data$y1), max(data$y1), length = biiter)
  )
  ...
  values <- list(var1 = var1, var2 = var2, data = data, plvar2=plvar2)
  print("Funktioniert.")
  return(data)
}
...
# TODO: FAZ ersetzen durch plvar2[1,]
# TODO: OOB einführen fürs plotten und printen
# TODO: plot.bivqfun()
